        ...
        for i in range(maxTime+1):               # 以花费时间为遍历序列
            ...
            for j in range(n):                   # 遍历每一个城市
                ...
                if currentFees == -1:            # 等于-1说明在当前时间还未到达该城市
                    continue
                # for k in range(n):               # 遍历每个城市，从j城到k城
                #     if graph[j][k] != -1:        # 如果j可到达k
                #         next = k                 # 更新从j到k时的金钱花费(当这样花费更少时，并且在时间允许范围内)
                #         time = i + graph[j][k]   # 计算到达k的时间
            ...
            #                 print("{}->{}:{}]".format(j, next, dp[time][next]), end=' ')
            # print()
